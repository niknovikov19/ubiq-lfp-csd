from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
from numpy import pi
import xarray as xr


# =============================================================================
# def plot_xarray_2d(X):
#     coord_x = X.dims[1]
#     coord_y = X.dims[0]
#     cx = X.coords[coord_x].values
#     cy = X.coords[coord_y].values
#     ext = [cx[0], cx[-1], cy[-1], cy[0]]
#     plt.imshow(X.values, aspect='auto', extent=ext, origin='upper')
#     plt.xlabel(coord_x)
#     plt.ylabel(coord_y)
# =============================================================================

def plot_xarray_2d(X, need_log=False, cmap=None, title=None, clim=None):
    coord_x = X.dims[1]
    coord_y = X.dims[0]
    cx = X.coords[coord_x].values
    cy = X.coords[coord_y].values
    Cx, Cy = np.meshgrid(cx, cy)
    ax = plt.gca()
    X_ = X.values
    if need_log:
        X_ = np.log(X_)
    if cmap is not None:
        cmap = plt.get_cmap(cmap)
    if clim is not None:
        vmin, vmax = clim[0], clim[1]
    else:
        vmin, vmax = None, None
    m = ax.pcolormesh(Cx, Cy, X_, shading='auto', cmap=cmap,
                      vmin=vmin, vmax=vmax)
    #ax.set_xscale('log')
    ax.invert_yaxis()
    plt.xlabel(coord_x)
    plt.ylabel(coord_y)
    plt.colorbar(m)
    if title is None:
        title = X.name
    plt.title(title)


dirpath_data = r'D:\WORK\Salvador\repo\ubiquitous-main\lfp_csd\data'

#fpath_in = Path(dirpath_data) / f'lfp_csd_avgtrial_3.nc'
fpath_in = Path(dirpath_data) / 'lfp_csd_avgtrial_(ref=0_n=5000_s=3).nc'
Xavg = xr.open_dataset(fpath_in)

#data_types_vis = ['lfp', 'csd', 'csd2']
#data_types_vis = ['lfp', 'csd2']
#data_types_vis = ['lfp', 'csd']
data_types_vis = ['lfp_interp', 'csd2_interp']

fbands = [(1, 10), (10, 30), (80, 100), (120, 140)]
colors = ['b--', 'b', 'r--', 'r']

nx = 4
ny = len(data_types_vis)

plt.figure(figsize=(12, 10))
for n, data_type in enumerate(data_types_vis):
    data_name = data_type[:3]
    # Power, 2-d
    plt.subplot(ny, nx, n * nx + 1)
    W = Xavg[f'{data_type}_pow']
    W /= W.max(dim='chan')
    plot_xarray_2d(W.T, need_log=True, title=f'{data_name}, power')
    # Phase, 2-d
    plt.subplot(ny, nx, n * nx + 2)
    Z = xr.apply_ufunc(np.angle, Xavg[f'{data_type}_complex'])
    plot_xarray_2d(Z.T, cmap='hsv', title=f'{data_name}, phase', clim=(-pi, pi))
    # Power, freq. bands
    plt.subplot(ny, nx, n * nx + 3)
    for fband, col in zip(fbands, colors):
        fband = fband[::-1]
        w = W.sel(freq=slice(*fband)).mean(dim='freq')
        #w /= w.max()
        plt.plot(w, w.chan, col, label=f'{fband[1]}-{fband[0]}')
    plt.gca().invert_yaxis()
    plt.legend()
    plt.title(f'{data_name}, power')
    # Phase, freq. bands
    plt.subplot(ny, nx, n * nx + 4)
    for fband, col in zip(fbands, colors):
        fband = fband[::-1]
        z = Z.sel(freq=slice(*fband)).mean(dim='freq')
        plt.plot(z, z.chan, col, label=f'{fband[1]}-{fband[0]}')
    plt.gca().invert_yaxis()
    plt.xlim(-pi, pi)
    plt.legend()
    plt.title(f'{data_name}, phase')

Xavg.close()
